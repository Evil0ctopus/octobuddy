...
Free Android pet companion featuring the **Evil0ctopus** brand octopus — with **XP, evolution stages, and an animated 3D pet** that now tracks the brand mark much more closely.
...
- **Brand-matched 3D pet** via SceneView / Filament (bundled `octobuddy.glb` rebuilt from `tools/generate_octobuddy_glb.py` to follow the Evil0ctopus mark: navy mantle, cyan glow eyes/rings, copper undersides + suckers); PNG fallback if load fails
- **Closer silhouette** vs 0.4.0: bulbous peaked head, slanted cyan eyes, eight radiating curled tentacles (was a purple/pink placeholder mesh)
- **XP + levels + evolution stages**
  ...
  - Stage changes visible scale + idle speed on the 3D model (scale-only on one mesh)
...
Application id: `com.evil0ctopus.octobuddy` · versionName **0.5.0** (versionCode 5)

### Verify on device (v0.5 checklist)

1. Fresh install → pet should appear as a **small (Hatchling) animated 3D octopus** with **navy body + cyan eyes** (gentle bob + spin), not a flat static PNG and not the old purple/pink mesh.
...
7. Optional: compare against `brand/brand-mark.jpg` / `drawable-nodpi/octobuddy_pet.png` — silhouette and colors should read as the same character family.
...
- **3D pet:** original brand-matched GLB (CC0), generated by [`tools/generate_octobuddy_glb.py`](tools/generate_octobuddy_glb.py) — see [`app/src/main/assets/models/README.md`](app/src/main/assets/models/README.md).
